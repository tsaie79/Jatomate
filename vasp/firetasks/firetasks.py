...
class Write2dNSCFKpoints(FiretaskBase):
    ...
    def run_task(self, fw_spec):
        ...
        if mode.lower() == "line":
            ...
            kpts = kpts
            kpts_weights = weights#[weights[kpt_idx] for kpt_idx in list(dict.fromkeys(kpt_ids))]
            labels = all_labels#[all_labels[kpt_idx] for kpt_idx in list(dict.fromkeys(kpt_ids))]
            num_kpts = len(kpts)
